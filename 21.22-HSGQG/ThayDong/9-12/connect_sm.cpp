...
int m, n, ans = oo;
int a[N][N], c[N][N], g[2][N][N], f[N][N][2][2];
queue<pi> q[10005];
struct cell{
    int d, x, y;
...
void dijkstra(int id){
    ...
    if ( !id )
        for (int i = 1; i <= m; i ++)
            g[id][i][1] = c[i][1], q[c[i][1]].push({i, 1});
    else
        for (int i = 1; i <= m; i ++)
            g[id][i][n] = c[i][n], q[c[i][n]].push({i, n});

    for (int d = 0; d <= 10000; d ++){
        while ( q[d].size() ){
            int x = q[d].front().fi, y = q[d].front().se;
            q[d].pop();
            if ( d != g[id][x][y] ) continue;
            for (int i = 0; i <= 3; i ++){
                int nx = x + row[i], ny = y + col[i];
                if ( nx < 1 || ny < 1 || nx > m || ny > n ) continue;
                if ( g[id][nx][ny] > d + c[nx][ny] ){
                    g[id][nx][ny] = d + c[nx][ny];
                    q[g[id][nx][ny]].push({nx, ny});
                }
            }
        }
    }
...
void anal(int s){
    ...
    for (int d = 0; d <= 10000; d ++){
        while ( heap[d].size() ){
            ...
            if ( !tl && f[x][y][1][tr] > f[x][y][tl][tr] + g[0][x][y] - c[x][y] ){
                f[x][y][1][tr] = f[x][y][tl][tr] + g[0][x][y] - c[x][y];
                heap[f[x][y][1][tr]].push({x, y, 1, tr});
            }

            if ( !tr && f[x][y][tl][1] > f[x][y][tl][tr] + g[1][x][y] - c[x][y] ){
                f[x][y][tl][1] = f[x][y][tl][tr] + g[1][x][y] - c[x][y];
