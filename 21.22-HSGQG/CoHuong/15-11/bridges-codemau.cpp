...
int main(){
    ...
    for (int i = 0; i < pt.size(); i++){
        int x = ub(pos.begin(), pos.end(),pt[i].x) - pos.begin();
        int y = ub(pos.begin(), pos.end(),pt[i].y) - pos.begin();
        update(x, pt[i].x);
        update(y, pt[i].y);
        val[x] = pt[i].x;
        val[y] = pt[i].y;
        ...
        pre[i] = 1ll * sma * val[cur] - ssma * 2 + tot - 1ll * (2 * (i + 1) - sma) * val[cur];
    }
    ...
    for (int i = pt.size() - 1; ~i; i--){
        ...
        int sma = get_cnt(cur - 1);
        ll ssma = get_sum(cur - 1);
        suf[i] = 1ll * sma * val[cur] - ssma * 2 + tot - 1ll * (2 * (pt.size() - i)- sma) * val[cur];
    }
    for (int i = 0; i < pt.size(); i++) res = min(res, pre[i] + suf[i + 1] + tmp);
    cout << res + pt.size() << '\n';
}
